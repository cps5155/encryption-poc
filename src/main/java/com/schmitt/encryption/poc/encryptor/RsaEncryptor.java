...
public class RsaEncryptor implements Encryptor {
    ...
    @Override
    public Optional<String> encrypt(String plaintextContent, Cipher encryptionCipher, AlgorithmParameterSpec algorithmParameterSpec) {
        try {
            encryptionCipher.init(Cipher.ENCRYPT_MODE, publicKey, algorithmParameterSpec);

            return encryptUsingInitializedCipher(plaintextContent, encryptionCipher);
        } catch (InvalidKeyException | InvalidAlgorithmParameterException e) {
            log.error("Caught exception attempting to initialize encryption cipher with Algorithm ParameterSpec: {}", e.getMessage());
        }
    ...
    private Optional<String> encryptUsingInitializedCipher(String plaintextContent, Cipher initializedCipher) {
        ...
        try {
            final byte [] encryptedContentBytes = initializedCipher.doFinal(plaintextContent.getBytes(StandardCharsets.UTF_8));
