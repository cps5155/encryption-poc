...
public abstract class AbstractEncryptionService implements EncryptionService {
    ...
    private final BiFunction<String, Cipher, Optional<String>> getEncryptionFunction =
            (plainTextContent, encryptionCipher) -> getAlgorithmParameterSpec()
                    .map(algorithmParameterSpec ->
                            getEncryptor().encrypt(plainTextContent, encryptionCipher, algorithmParameterSpec))
    ...
    private final BiFunction<String, Cipher, Optional<String>> getDecryptionFunction =
            (encryptedContent, decryptionCipher) -> getAlgorithmParameterSpec()
                    .map(algorithmParameterSpec ->
                            getDecryptor().decrypt(encryptedContent, decryptionCipher, algorithmParameterSpec))
    ...
    public boolean encrypt(String plainTextFileContent, File persistenceLocation) {
        return getCipherFactory()
                .getCipher()
                .flatMap(encryptionCipher -> getEncryptionFunction.apply(plainTextFileContent, encryptionCipher))
                .map(encryptedResult -> writeContentToFile(
                        persistenceLocation, encryptedResult.getBytes(), "Error writing encrypted content to file"))
                .orElseGet(() -> {
                    log.warn(
                            "Encrypted content persistence failure - no content returned from encryptor, nothing will be persisted");
    ...
    public boolean decrypt(String encryptedFileContent, File persistenceLocation) {
        return getCipherFactory()
                .getCipher()
                .flatMap(decryptionCipher -> getDecryptionFunction.apply(encryptedFileContent, decryptionCipher))
                .map(decryptedResult -> writeContentToFile(
                        persistenceLocation, decryptedResult.getBytes(), "Error writing decrypted content to file"))
                .orElseGet(() -> {
                    log.warn(
                            "Decrypted content persistence failure - no content returned from decryptor, nothing will be persisted");
